...
#[cfg(not(feature = "std"))]
compile_error!("asimov-anthropic-prompter requires the 'std' feature");
...
pub fn main() -> Result<SysexitsError> {
    ...
    let Ok(manifest) = asimov_module::ModuleManifest::read_manifest("anthropic")
    ...
    let options = asimov_anthropic_module::Options::builder()
        .endpoint(endpoint)
        .model(model)
        .api_key(api_key)
        .build();
    ...
    let response =
        asimov_anthropic_module::chat(&input, &options).context("failed to generate response")?;
